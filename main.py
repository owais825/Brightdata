...
async def run_agent(query):
        ...
        client = MultiServerMCPClient(
            {
                "bright_data": {
                    ...
                    "args": ["@brightdata/mcp"],
                    "env": {
                        "API_TOKEN": os.getenv("BRIGHT_DATA_API_TOKEN") or ""
                    },
                    "transport": "stdio",
                },
            }
    ...
    stock_finder_agent = create_react_agent(
        ...
        prompt=""" You are a stock research analyst specializing in the Indian Stock Market (NSE). 
                    Your task is to select 2 promising, actively traided NSE-listed stocks for 
                    short term trading (buy/sell) based on recent performance, news buzz,volume or technical strength.
    ...
    final_messages = convert_to_messages(
        final_chunk["supervisor"]["messages"]
    )

    print("\nFINAL OUTPUT:\n")
    pretty_print(final_messages[-1])


if __name__ == "__main__":
    asyncio.run(
        run_agent("Give me NSE stock recommendations")
    )